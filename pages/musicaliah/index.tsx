...
import { FormMusicPassword } from '@/src/components/FormMusicPassword';

const Music: NextPage = () => {
  const { t } = useTranslation('music');

  const methods = useForm<MusicFormFields & { password: string }>({
    mode: 'onChange',
  });
  ...
  const [isDurationCorrect, setIsDurationCorrect] = useState(true);
  ...
  const onSubmit = useCallback(
    async (data: MusicFormFields & { password: string }) => {
      if (data.password !== config.other.adminPin) return;
      setIsLoading(true);
